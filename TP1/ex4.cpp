...
void dfsAuxEx4(Vertex* v, std::vector<int> &res, bool &cycle){
    v->setIndegree(1);
    for(auto w : v->getAdj()){
        if(w->getDest()->getIndegree() == 1){
            cycle = true;
        }
        else if(w->getDest()->getIndegree() == 0)
            dfsAuxEx4(w->getDest(), res, cycle);
    }
    v->setIndegree(2);
